...
class MyPiMQTTClient():
    ...
    def _subscribe_update_callback(self, client, userdata, message):
        ...
        try:
            ...
            print("GONNA TRY IT")
            remote_data_timestamp = payload_dict['current']['state']['desired']['timestamp']
            if self.remote.needs_updating(remote_data_timestamp):
                remote_data= payload_dict['current']['state']['desired'][remote_data_timestamp]
                self.remote.update_remote(remote_data)
                reported_payload = {
                                       'state': {
                                        'reported': self.remote.current_settings()
                                       }
                                   }
    ...
    def run_app(self, set_desired_state=False):
        ...
        start_payload = {
                            'state': {
                                'reported': self.remote.current_settings()
#                                'desired': self.remote.current_settings()
                            }
                        }
...
def main():
    ...
    my_pi_mqtt_client = MyPiMQTTClient()
    my_pi_mqtt_client.run_app()
